...
import {
    Wand2, Calendar as CalendarIcon, AlertTriangle,
    CheckCircle2, Users, ArrowRight, RefreshCw, XCircle
} from "lucide-react";
        ...
        <div className="space-y-6 animate-fadeUp h-full flex flex-col">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shadow-sm pb-4 border-b border-border">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1 text-sm font-medium">Модуль ИИ-генерации (Потоки, замены, кабинеты)</p>
                </div>
                <button
                    onClick={handleGenerate}
                    disabled={isGenerating || generated}
                    className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 shadow-sm ${isGenerating
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1">
                ...
                <div className="lg:col-span-2 liquid-glass rounded-[2rem] p-6 flex flex-col relative overflow-hidden">
                    ...
                    {!generated && !isGenerating && (
                        <div className="absolute inset-0 z-10 bg-white/40 backdrop-blur-[2px] flex items-center justify-center">
                            <div className="bg-white/90 p-6 rounded-2xl shadow-xl border border-primary/20 text-center max-w-sm">
                                ...
                                <p className="text-sm text-muted-foreground leading-relaxed">Система ИИ распределит 340 уроков по 40 классам без накладок, учитывая отсутствия и разделения на подгруппы.</p>
                            </div>
                        </div>
                    )}
                    ...
                    {isGenerating && (
                        <div className="absolute inset-0 z-10 bg-white/60 backdrop-blur-md flex items-center justify-center">
                            <div className="text-center space-y-4">
                                ...
                                <p className="font-sora font-semibold text-lg text-primary animate-pulse">Оптимизация графа расписания...</p>
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-sora font-bold text-foreground">Сетка: 10 классы (Понедельник)</h2>
                        <div className="flex gap-2">
                            <button className="text-xs font-bold px-3 py-1 bg-black/5 rounded-lg text-foreground">10 А</button>
                            <button className="text-xs font-bold px-3 py-1 bg-transparent border border-border rounded-lg text-muted-foreground">10 Б</button>
                            <button className="text-xs font-bold px-3 py-1 bg-transparent border border-border rounded-lg text-muted-foreground">10 В</button>
                        </div>
                    </div>
                ...
                {/* Right Col - AI Assistant & Logs */}
                <div className="lg:col-span-1 space-y-4">
                    <h2 className="font-sora font-bold text-foreground">Анализ коллизий (ИИ)</h2>
                        ...
                        {generated ? (
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <p className="text-sm font-medium text-green-900 leading-snug">0 накладок. Все 40 классов укомплектованы на 100%.</p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="bg-orange-50 border border-orange-100 p-2 rounded-lg text-xs text-orange-800 flex flex-col gap-1">
                                        ...
                                        <span>ИИ объединил 10А и 10Б на стадионе со вторым учителем (Иванов В.П.).</span>
                                    </div>
                                    <div className="bg-blue-50 border border-blue-100 p-2 rounded-lg text-xs text-blue-900 flex flex-col gap-1">
                                        ...
                                        <span>Все уроки переведены на поле на улице согласно погоде.</span>
                                    </div>
                                </div>

                                <button className="w-full bg-black/5 hover:bg-black/10 text-foreground py-2 rounded-xl text-xs font-bold transition-colors mt-2">
